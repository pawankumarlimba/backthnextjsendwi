...
id
    ...
    return (
                ...
                {Allarts.map((user, index) => (
                    <div className="flex justify-center" key={index}>
                        <BackgroundGradient className="rounded-[22px] w-[80vw] sm:w-[25vw] p-4 sm:p-10 bg-white dark:bg-zinc-900">
                            <div>
                                <h2 className="text-xl font-bold mb-1 truncate w-[150px]">{user.name}</h2>
                                <Image
          src={`/${user.url}`}
          alt="jordans"
          height="400"
        ...
        />
                                <p className="text-gray-600 truncate w-[150px]">{user.description}</p>
                                <p className="text-gray-600 truncate w-[150px]">{user.price}</p>
                            </div>
                            <button onClick={() => handleartremove(user._id)} className="rounded-full pl-4 pr-1 py-1 text-white flex items-center space-x-1 bg-black mt-4 text-xs font-bold dark:bg-zinc-800">
